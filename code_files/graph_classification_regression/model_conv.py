...
from torch_geometric.nn import global_mean_pool, global_add_pool, SAGEConv, GATConv
...
class GNN_node_Virtualnode(torch.nn.Module):
    def __init__(self, num_layer, emb_dim, drop_ratio=0.5, JK="last", residual=False, gnn_type='gin', deg=None):
        ...
        for layer in range(num_layer):
            if gnn_type == 'gin':
                ...
                self.convs.append(GATConv(emb_dim, emb_dim, heads=1, concat=False))
            # elif gnn_type == 'pna':
    ...
    def forward(self, batched_data):
        ...
        for layer in range(self.num_layer):
            ...
            # Handle edge_attr only if conv supports it
            if isinstance(conv, (GINConv, GCNConv)):
                h = conv(h_list[layer], edge_index, edge_attr)
            else:  # For GAT, SAGE (no edge_attr)
                h = conv(h_list[layer], edge_index)
