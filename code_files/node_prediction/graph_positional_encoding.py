...
def print_feature_stats(name, tensor):
    min_val = tensor.min(0).values
    max_val = tensor.max(0).values
    mean_val = tensor.mean(0)
    std_val = tensor.std(0)
    q1 = tensor.quantile(0.01, dim=0)
    q99 = tensor.quantile(0.99, dim=0)

    print(f"=== {name} stats ===")
    print(f"Min:  {min_val.min().item():.4f}  | Max:  {max_val.max().item():.4f}")
    print(f"Mean: {mean_val.mean().item():.4f} | Std:  {std_val.mean().item():.4f}")
    print(f"1st percentile: {q1.mean().item():.4f} | 99th percentile: {q99.mean().item():.4f}")
    print(f"Max - Min (range): {(max_val - min_val).mean().item():.4f}")
    print(f"Outlier spread (Q99 - Q1): {(q99 - q1).mean().item():.4f}")
    print()
...
def get_graph_info(args, lap_save_dir, dataset, d, undir_edge_index, device):
    
    node_mean_abs = torch.mean(torch.abs(dataset.graph["node_feat"]))
    if(args.encode_mode == "lap"):
        ...
        if(args.agg_mode == "add"):
            ...
            x_agg = torch.cat([dataset.graph['node_feat'], normalized_eigvecs], dim=1)
        else:
            x_agg = dataset.graph["node_feat"]
        ...
        if (os.path.exists(multihop_save_path) and os.path.exists(lap_save_path)):
            ...
            with torch.no_grad():
                ...
                if(os.path.exists(multihop_save_path)):
                    ...
                    if(args.dataset == "ogbn-papers100M"):
                        x_1hop= aggregate_hops_mean(dataset.graph["node_feat"], dataset.graph["edge_index"], device, args)
                        # Save the computed features
                        multihop_features = {
                            'x_1hop': x_1hop,
                        }
                    else:
                        x_1hop, x_2hop = aggregate_hops_mean(dataset.graph["node_feat"], dataset.graph["edge_index"], device, args)
                        # Save the computed features
                        multihop_features = {
                            'x_1hop': x_1hop,
                            'x_2hop': x_2hop,
        ...
        if(args.dataset == "ogbn-papers100M"):
            x_1hop = x_1hop.to(dataset.graph["node_feat"].device)
            x_agg = (dataset.graph["node_feat"] +  x_1hop) / 2
            ...
            x_1hop = x_1hop.to(dataset.graph["node_feat"].device)
            x_2hop = x_2hop.to(dataset.graph["node_feat"].device)
            x_agg = (dataset.graph["node_feat"] +  x_1hop + x_2hop) / 3
            # print_feature_stats("x_agg", x_agg)
            # print_feature_stats("lap_pe", lap_pe)
