...
from graph_positional_encoding import get_graph_info
import random 
import gc
import time
...
def main():
    ...
    for run in range(args.runs):
        ...
        model.load_state_dict(torch.load(checkpoint_path)['model_state_dict'], strict=True)
        model.eval()
        split_idx = split_idx_lst[run]

        result = evaluate(model, dataset, split_idx, eval_func, criterion, args, x_agg=x_agg)
