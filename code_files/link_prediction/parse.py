...
def parse_method(args, c, d, n, device):
    ...
    if args.method == 'gcn':
        ...
        model = SAGE(in_channels=d,
                    hidden_channels=args.hidden_channels,
                    out_channels=c,
                    num_layers=args.gnn_num_layers,
                    dropout=args.gnn_dropout,
                    ).to(device)
    elif args.method == "gcnwithjk":
        model = GCNwithJK(in_channels=d,
                    hidden_channels=args.hidden_channels,
                    out_channels=c,
                    num_layers=args.gnn_num_layers,
                    dropout=args.gnn_dropout,
                    use_bn=args.gnn_use_bn, use_ln=args.gnn_use_ln,normalize=args.gnn_normalize, cached=args.gnn_cached
                    ).to(device)
        ...
        model = SGC(in_channels=d,
                    ...
                    num_layers=args.gnn_num_layers,
                    dropout=args.gnn_dropout,
                    use_bn=args.gnn_use_bn, use_ln=args.gnn_use_ln
                    ).to(device)
    elif args.method == 'SAGMM':
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--gnn_normalize', action='store_true', help='use normalize for GCN')
    parser.add_argument('--gnn_cached', action='store_true', help='use cached for GCN')
    ...
    parser.add_argument('--add_aux_loss',action='store_true', help='To decided whether to add auxiliary loss')
