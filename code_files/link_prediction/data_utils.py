...
def eval_acc_condense(y_true, y_pred):
    acc_list = []
    y_true = y_true.detach().cpu().numpy()
    y_pred = y_pred.argmax(dim=-1, keepdim=True).detach().cpu().numpy()
    ...
    for i in range(y_true.shape[0]):
        ...
        correct = y_true[i] == y_pred[is_labeled, i]
...
def to_pyg_graph(adjacency_matrix, feature_matrix, labels):
    edge_index = torch.nonzero(adjacency_matrix, as_tuple=False).t().contiguous()
    return Data(x=feature_matrix, edge_index=edge_index, y=labels)
